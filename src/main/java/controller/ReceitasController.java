        ...
        if (userId != null) {
            ...
            if (account != null) {
                if (session.getAttribute("accId") == null) {
                    ...
                    System.out.println("accId da conta: " + account.getAccId());
                }

                try {
                    List<Transaction> allTransactions = transactionDao.getAllByAccount(account.getAccId());
                    List<Transaction> receitas = allTransactions.stream()
                            .filter(transaction -> "R".equals(transaction.getType()))
                            .collect(Collectors.toList());

                    request.setAttribute("transactions", receitas);
                    request.getRequestDispatcher("/receitas/receitas.jsp").forward(request, response);
                } catch (Exception e) {
